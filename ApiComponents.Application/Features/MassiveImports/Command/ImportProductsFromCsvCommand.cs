...
namespace ApiComponents.Application.Features.MassiveImports.Command;
...
public class ImportProductsFromCsvCommandHandler : IRequestHandler<ImportProductsFromCsvCommand, ImportResultDto>
{
    ...
    public ImportProductsFromCsvCommandHandler(IProductRepository productRepo)
    {
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------
    // funciÃ³n: Handle()
    // - Se encarga de coordinar todo el flujo de importaciÃ³n de productos desde un archivo CSV.
    // - Configura el lector, valida la estructura del encabezado, detecta la categorÃ­a de forma dinÃ¡mica y ejecuta las validaciones fila por fila.
    // - Si una fila contiene datos invÃ¡lidos o duplicados, acumula el detalle del error especificando el nÃºmero de fila afectado en una lista, 
    // permitiendo que el proceso continÃºe con las siguientes lÃ­neas y cancelando la persistencia solo si se detectÃ³ al menos un fallo.
    //----------------------------------------------------------------------------------------------------------------------------------------
        ...
        try
        {
            ...
            while (await csv.ReadAsync())
            {
                try
                {
                    ...
                    // 1. VALIDACIÃ“N Y DETECCIÃ“N DE CATEGORÃA
                    ...
                    // 2. VALIDACIONES BASE
                    if (await _productRepo.ExistProduct(title))
                    {
                        result.Errors.Add($"Fila {rowNumber}: El producto '{title}' ya existe.");
                        rowNumber++; continue;
                    }

                    // 3. MAPEO E INICIALIZACIÃ“N DEL PRODUCTO (Llamada al mÃ©todo extraÃ­do)
                    var product = MapProductFromCsv(csv, title, currentCatId);

                    // 4. PROCESAR ATRIBUTOS EXTRA (Llamada al mÃ©todo extraÃ­do)
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------
    // funciÃ³n: MapProductFromCsv()
    // - Mapea de forma manual y explÃ­cita los datos de la fila actual del CSV a una nueva entidad <see cref="Product"/>.
    // - Se encarga de parsear los tipos numÃ©ricos usando una cultura invariante, aplicar valores por defecto
    // - y procesar las sub-colecciones separadas por comas (ImÃ¡genes y Tags).
    //----------------------------------------------------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------
    // FunciÃ³n ProcessExtraAttributes: Este mÃ©todo se encarga de:
    // - Procesa las columnas dinÃ¡micas (atributos extra) del CSV que pertenecen a la categorÃ­a detectada.
    // - Valida que el tipo de dato ingresado (NÃºmero, Booleano, etc.) coincida con la definiciÃ³n de la base de datos
