...
public class ProductService(IProductRepository productRepo, AppDbContext context) : IProductService
{
    public async Task<ImportResultDto> ProcessCsvAsync(IFormFile file)
    {
        ...
        // Variables para la detección dinámica de la categoría
        ...
        try
        {
            ...
            while (await csv.ReadAsync())
            {
                try
                {
                    ...
                    // Verificamos si el campo está vacío (es obligatorio)
                    ...
                    // Lógica de detección inicial (Se ejecuta solo una vez con la primera fila válida)
                    if (detectedCategoryId == null)
                    {
                        detectedCategoryId = currentCatId;

                        // Buscamos la categoría. EF devolverá un Proxy, pero accederemos a .name sin problemas.
                        ...
                        // Cargamos las definiciones de atributos extra
                        allowedDefinitions = await context.ProductAttributeDefinitions
                            .Where(d => d.categoryId == detectedCategoryId)
                            .ToListAsync();
                    }
                    // Verificamos consistencia (que no mezclen categorías en el mismo CSV)
                    ...
                    // --- 3. CREACIÓN DEL OBJETO PRODUCTO ---
                    var product = new Product
                    {
                        ...
                        attributeValues = new List<ProductExtraAttributeValue>(),
                    ...
                    // 4. ATRIBUTOS EXTRA (Solo si existen columnas extra)
                    foreach (var header in extraHeadersInCsv)
                    {
                        ...
                        // Deserializar validaciones
                        var valConfig = string.IsNullOrEmpty(definition.validationsJson)
                            ? new ExtraAttributeValidationsDto()
                            : System.Text.Json.JsonSerializer.Deserialize<ExtraAttributeValidationsDto>(definition.validationsJson) ?? new ExtraAttributeValidationsDto();

                        // Aplicar Switch de validación que definimos antes
                        bool isValid = true;
                        switch (definition.dataType?.ToLower())
                        {
                            case "number":
                                if (!double.TryParse(rawValue, out _)) { result.Errors.Add($"Fila {rowNumber}: '{header}' no es número."); isValid = false; }
                                break;
                            case "boolean":
                                var low = rawValue.ToLower();
                                if (low == "true" || low == "1" || low == "si") rawValue = "true";
                                else if (low == "false" || low == "0" || low == "no") rawValue = "false";
                                else { result.Errors.Add($"Fila {rowNumber}: '{header}' no es booleano."); isValid = false; }
                                break;
                        }
