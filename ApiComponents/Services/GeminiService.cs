...
namespace ApiComponents.Services
{
    public class GeminiService : IGeminiService
    {
        ...
        /// - cancellationToken: Token de cancelación para interrumpir de forma segura las consultas a la base de datos o la IA si la petición expira.
        /// 
        /// QUÉ HACE:
        ...
        private async Task<List<ProductMatch>> GetSemanticMatchesFromAI(string userQuestion, string catalogJson, List<ProductDto> products, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogError(ex, "Error parsing 'matches' from AI response. Forcing local fallback.");
                    return ExecuteLocalFallbackMatching(userQuestion, products);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error comunicándose con el repositorio de Gemini. Forcing local fallback.");
                return ExecuteLocalFallbackMatching(userQuestion, products);
            }
        }
        ...
        // Esta función es para cuando el usuario ya tiene tiene el producto/os en el componente ia-chat del frontend y quiere hacer una pregunta de un producto de esa lista.
        ...
        /// </summary>
        private string BuildSemanticPrompt(string userQuestion, string catalogJson)
        {
        ...
        /// </summary>
        private string ExtractJsonArray(string cleanJson)
        {
        ...
        /// Si el JSON de entrada no contienecoincidencias o es inválido, devuelve una lista limpia con cero elementos (vacía).
        /// </summary>
        private List<ProductMatch> ParseMatchesFromJson(string arrayJson)
        {
        ...
        /// </summary>
        private ProductMatch? MapElementToProductMatch(JsonElement el)
        {
            int id = 0;
            double score = 0;
        ...
        /// </summary>
        private List<ProductMatch> ExecuteLocalFallbackMatching(string userQuestion, List<ProductDto> products)
        {
